...
async fn update_task(app_state: web::Data<AppState>, id: web::Path<u64>, task: web::Json<Task>) -> impl Responder {
    let path_id = id.into_inner();
    let task = task.into_inner();

    if task.id != path_id {
        return HttpResponse::BadRequest().body("Invalid request, IDs do not match.");
    }
